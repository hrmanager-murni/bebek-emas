...
window.thermalPrinter = null; window.lastPrintedPayload = null;
...
window.connectPrinter = async () => {
    try {
        ...
        const server = await device.gatt.connect(); window.thermalPrinter = server; showToast("Printer " + device.name + " terhubung!", "success");
        const btn = document.getElementById('btnPrinterStatus'); btn.innerHTML = `<i class="fas fa-print"></i><span class="hidden sm:inline ml-1">PRINTER AKTIF</span>`; btn.classList.replace('bg-slate-800', 'bg-emerald-500/20'); btn.classList.replace('text-slate-300', 'text-emerald-400'); btn.classList.replace('border-slate-700', 'border-emerald-500/50');
    } catch (err) { console.error(err); showToast("Batal hubungkan printer.", "error"); }
};

window.printReceiptAction = async () => {
    if (window.thermalPrinter && window.thermalPrinter.connected && window.lastPrintedPayload) {
        try {
            showToast("Mencetak...", "info"); const services = await window.thermalPrinter.getPrimaryServices();
            if (services.length > 0) {
                const service = services[0]; const characteristics = await service.getCharacteristics();
                if (characteristics.length > 0) {
                    const char = characteristics.find(c => c.properties.write || c.properties.writeWithoutResponse) || characteristics[0];
                    let enc = new TextEncoder(); let p = window.lastPrintedPayload; let escpos = [0x1B, 0x40, 0x1B, 0x61, 0x01]; 
                    ...
                    if(p.pembayaran === 'MIX') escpos.push(...enc.encode(`Bayar   : MIX (C:${formatIDRPlain(p.mixCash)} T:${formatIDRPlain(p.mixTf)})\n`));
                    ...
                    for (let i = 0; i < data.length; i += 20) { if (char.properties.writeWithoutResponse) await char.writeValueWithoutResponse(data.slice(i, i + 20)); else await char.writeValue(data.slice(i, i + 20)); }
                    showToast("Selesai mencetak!", "success"); return; 
                }
            }
...
async function initApp() { showLoading("MENGAMANKAN KONEKSI CLOUD..."); try { await signInAnonymously(auth); await seedInitialAccounts(); setupRealtimeSync(); } catch (err) { console.error(err); showToast("Koneksi gagal.", "error"); hideLoading(); } }

async function seedInitialAccounts() {
    const snap = await getDocs(getColRef('accounts'));
    if (snap.empty) {
        const defaults = [ { id: 'admin', data: { username: 'admin', pass: 'bosbebek40', nama: 'Manager Executive', role: 'admin' } }, { id: 'kasir1', data: { username: 'kasir1', pass: '123', nama: 'Risma', role: 'kasir' } }, { id: 'koki1', data: { username: 'koki1', pass: '123', nama: 'Kepala Koki', role: 'koki' } }, { id: 'barista1', data: { username: 'barista1', pass: '123', nama: 'Barista', role: 'barista' } }, { id: 'waiter1', data: { username: 'waiter1', pass: '123', nama: 'Waiter', role: 'waiter' } }, { id: 'staff_dapur1', data: { username: 'staff_dapur1', pass: '123', nama: 'Staff Dapur', role: 'staff_dapur' } } ];
...
function setupRealtimeSync() {
    ...
    const handleSync = (col) => {
        ...
        if (['transactions', 'expenses', 'stock_mutations', 'close_registers'].includes(col)) dbRef = query(getColRef(col), where('tanggal', '>=', thirtyDaysAgo));
...
window.tarikDataArsip = async () => {
    ...
    try {
        const d = new Date(); d.setDate(d.getDate() - 30); const thirtyDaysAgo = d.toISOString().split('T')[0];
        const fetchArchive = async (col) => { const q = query(getColRef(col), where('tanggal', '<', thirtyDaysAgo)); const snap = await getDocs(q); window.arsipData[col] = snap.docs.map(docItem => ({id: docItem.id, ...docItem.data()})); };
        ...
        if(btn) { btn.innerHTML = '<i class="fas fa-check-circle text-lg"></i><span class="ml-2">ARSIP AKTIF</span>'; btn.classList.replace('bg-slate-900', 'bg-emerald-600'); btn.classList.replace('text-amber-400', 'text-white'); btn.classList.replace('border-slate-700', 'border-emerald-500'); btn.disabled = true; }
    } catch (e) { console.error(e); showToast("Gagal menarik arsip kuno.", "error"); }
    hideLoading();
};
...
window.loginAs = async (username, role) => {
    ...
    if (!acc) { let displayName = "Staff " + username; if(role==='kasir') displayName = "Risma"; else if(role==='koki') displayName="Kepala Koki"; const payload = { username: username, pass: '123', nama: displayName, role: role }; await setDoc(getDocRef('accounts', username), payload); acc = { id: username, ...payload }; }
    proceedLogin(acc);
};
...
window.loginManager = async () => {
    ...
    if (pass === adminAcc.pass || pass === 'bosbebek40') { document.getElementById('modalManagerPass').classList.add('hidden'); proceedLogin(adminAcc); } else { showToast('Otorisasi gagal!', 'error'); }
};
...
function proceedLogin(accountData) {
    ...
    if (currentUser.role === 'admin') {
        // HAPUS 'laporan' dari akses menu Manager
...
window.bukaModalSimpanNota = () => { 
    ...
    let cust = document.getElementById('cartCustomer').value; let meja = document.getElementById('cartMeja').value; let autoName = "";
    if(cust && meja) autoName = cust + " - " + meja; else if (cust) autoName = cust; else if (meja) autoName = "Meja " + meja;
    document.getElementById('inputParkName').value = autoName; document.getElementById('modalParkBill').classList.remove('hidden'); 
};
...
window.renderKategoriFilterKasir = () => {
    const container = document.getElementById('containerFilterKategoriKasir'); if(!container) return;
    const standardKategories = ['MAKANAN', 'MINUMAN', 'TAMBAHAN']; let allKats = [...new Set(menusDB.map(m => m.kategori.toUpperCase()))]; allKats = [...new Set([...standardKategories, ...allKats])]; 
    ...
    allKats.forEach(kat => { html += `<button onclick="setFilterKategoriKasir('${kat}')" id="btnKat-${kat}" class="btn-kat-kasir px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition whitespace-nowrap ${activeKategoriKasir === kat ? 'bg-amber-50 text-amber-600 shadow-sm border border-amber-200' : 'text-slate-500 hover:bg-slate-50'}">${kat}</button>`; });
...
window.bukaMixPay = () => {
    ...
    let totalAfterDisc = subtotalCart - discountInfo.amount; if (tipeOrder === 'Gojek') { const g = getGojekFee(totalAfterDisc); totalAfterDisc = g.finalTotal; }
    document.getElementById('mixTotalTagihan').innerText = formatIDR(totalAfterDisc); document.getElementById('mixInputCash').value = ''; document.getElementById('mixSisaTf').innerText = formatIDR(totalAfterDisc); document.getElementById('modalMixPay').classList.remove('hidden');
};

window.hitungMixTf = () => {
    let totalAfterDisc = subtotalCart - discountInfo.amount; if (tipeOrder === 'Gojek') { const g = getGojekFee(totalAfterDisc); totalAfterDisc = g.finalTotal; }
    let cashPay = getCleanNumber(document.getElementById('mixInputCash').value); let sisa = totalAfterDisc - cashPay; if (sisa < 0) sisa = 0; document.getElementById('mixSisaTf').innerText = formatIDR(sisa);
};

window.prosesMixPay = () => {
    let cashPay = getCleanNumber(document.getElementById('mixInputCash').value); let totalAfterDisc = subtotalCart - discountInfo.amount; if (tipeOrder === 'Gojek') { const g = getGojekFee(totalAfterDisc); totalAfterDisc = g.finalTotal; }
    let tfPay = totalAfterDisc - cashPay; if (cashPay <= 0 || tfPay <= 0) return showToast("Masukan Nominal Mix Pay Tidak Valid!", "error"); document.getElementById('modalMixPay').classList.add('hidden'); prosesBayar('MIX', cashPay, tfPay);
};
...
function cetakStrukPreview(payload) {
    document.getElementById('strukNota').innerText = "Nota: #" + payload.noNota; document.getElementById('strukWaktu').innerText = payload.waktu; document.getElementById('strukKasir').innerText = "Kasir: " + payload.kasir; document.getElementById('strukCustomerDetail').innerText = `${payload.customer} / ${payload.meja}`; document.getElementById('strukTipe').innerText = payload.tipeOrder; 
    if(payload.pembayaran === 'MIX') document.getElementById('strukMetode').innerText = `MIX (C: ${formatIDRPlain(payload.mixCash)} / T: ${formatIDRPlain(payload.mixTf)})`; else document.getElementById('strukMetode').innerText = payload.pembayaran;
    const itemsContainer = document.getElementById('strukItems'); itemsContainer.innerHTML = ''; payload.cart.forEach(item => { const harga = payload.tipeOrder === 'DineIn' ? item.hargaDineIn : item.hargaGojek; itemsContainer.innerHTML += `<div class="flex justify-between items-start"><span class="max-w-[160px] leading-tight">${item.nama} <span class="lowercase text-slate-400 font-bold">x${item.qty}</span></span><span class="font-bold">${formatIDR(harga * item.qty)}</span></div>`; });
    document.getElementById('strukSubtotal').innerText = formatIDR(payload.subtotal); document.getElementById('strukDiskon').innerText = `- ${formatIDR(payload.diskonRp)}`; 
    if(payload.tipeOrder === 'Gojek' && payload.gojekFee) { document.getElementById('strukGojekRow').classList.remove('hidden'); document.getElementById('strukGojekFee').innerText = `- ${formatIDR(payload.gojekFee)}`; } else { document.getElementById('strukGojekRow').classList.add('hidden'); }
    let totalCetak = payload.totalBayar !== undefined ? payload.totalBayar : (payload.subtotal - payload.diskonRp); document.getElementById('strukTotal').innerText = formatIDR(totalCetak); document.getElementById('modalStruk').classList.remove('hidden');
}
...
window.renderTutupBukuUI = () => {
    if(currentUser && currentUser.role === 'admin') {
        ...
        if(!document.getElementById('filterTbManager').value) { const d = new Date(); document.getElementById('filterTbManager').value = `${d.getFullYear()}-${(d.getMonth()+1).toString().padStart(2,'0')}`; } window.renderManagerTutupBuku();
    } else {
...
window.bukaKunciTbManager = async (id) => { showLoading("Membuka Kunci..."); try { await setDoc(getDocRef('close_registers', id), { isUnlocked: true }, { merge: true }); hideLoading(); document.getElementById('modalDetailTbManager').classList.add('hidden'); showToast("Akses revisi diberikan ke Kasir!", "success"); renderManagerTutupBuku(); } catch (err) { console.error(err); hideLoading(); showToast("Gagal membuka kunci", "error"); } };
...
// FEEDBACK 2: RENDER KELOLA STOK TOKEN/KARTU
window.renderKelolaStok = () => { 
    const container = document.getElementById('containerKelolaStok'); container.innerHTML = ''; 
...
window.loadMutasiDataUI = () => {
    if(currentUser && currentUser.role === 'admin') {
        ...
        if(isLocked) { 
            document.getElementById('btnSubmitMutasi').classList.add('hidden'); document.getElementById('mutasiLockedAlert').classList.remove('hidden'); 
            const btnAjukan = document.getElementById('btnAjukanBuka');
            if(sudahRequest) { btnAjukan.innerText = "Menunggu Persetujuan Manager..."; btnAjukan.disabled = true; btnAjukan.classList.add('opacity-50', 'cursor-not-allowed', 'bg-slate-400'); btnAjukan.classList.remove('bg-rose-500', 'hover:bg-rose-600'); } 
        ...
        } else { document.getElementById('btnSubmitMutasi').classList.remove('hidden'); document.getElementById('mutasiLockedAlert').classList.add('hidden'); }
        ...
        targetStocks.forEach(stokItem => {
            ...
            const masuk = dataTersimpan ? dataTersimpan.masuk : (dLocal.masuk || 0); const manual = dataTersimpan ? dataTersimpan.manual : (dLocal.manual || 0); const rusak = dataTersimpan ? dataTersimpan.rusak : (dLocal.rusak || 0); 
            const seharusnya = formatDec(sisaKemarin + masuk - potongKasir - manual - rusak); const fisik = dataTersimpan ? (dataTersimpan.fisik !== null ? dataTersimpan.fisik : '') : (dLocal.fisik !== undefined ? dLocal.fisik : ''); const selisih = (fisik !== '') ? formatDec(fisik - seharusnya) : 0;
...
window.renderMutasiTable = () => {
    ...
    mutasiDraftHariIni.forEach((m, idx) => {
        ...
        const dis = m.isLocked ? 'disabled' : ''; const inputClass = `w-full text-center bg-white border border-slate-200 rounded-lg p-2 font-black outline-none transition shadow-sm text-slate-700 ${m.isLocked ? 'opacity-50' : 'focus:ring-2 focus:ring-blue-400'}`;
...
window.renderRekapMenuTab = () => {
    const sd = document.getElementById('rmStartDate').value; const ed = document.getElementById('rmEndDate').value; if(!sd || !ed) { const dEnd = new Date(); const dStart = new Date(); dStart.setDate(dEnd.getDate() - 6); document.getElementById('rmStartDate').value = dStart.toISOString().split('T')[0]; document.getElementById('rmEndDate').value = dEnd.toISOString().split('T')[0]; setTimeout(window.renderRekapMenuTab, 50); return; }
    let itemSales = {}; let itemRevenue = {}; const start = new Date(sd).getTime(); const end = new Date(ed).getTime() + 86400000;
    transactionsDB.filter(t => t.status === 'Aktif').forEach(t => { const tTime = new Date(t.tanggal).getTime(); if(tTime >= start && tTime < end) { t.cart.forEach(c => { itemSales[c.nama] = (itemSales[c.nama] || 0) + c.qty; const harga = t.tipeOrder === 'DineIn' ? c.hargaDineIn : c.hargaGojek; itemRevenue[c.nama] = (itemRevenue[c.nama] || 0) + (harga * c.qty); }); } });
